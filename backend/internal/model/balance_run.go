...
type BalanceRun struct {
	...
	BalanceStatus        constants.BalanceStatus  `json:"balance_status" gorm:"type:varchar(24);not null;check:balance_status IN ('queued','calculating','pending_review','accepted','rejected','invalidated','recalculation_required','replaced')"`
	...
	RecalculationReasons datatypes.JSON           `json:"recalculation_reasons" gorm:"type:jsonb;not null;default:'[]'"`
	SupersedesID         *uint                    `json:"supersedes_id" gorm:"index"`
	SupersededByID       *uint                    `json:"superseded_by_id" gorm:"index"`
	...
	Supersedes           *BalanceRun              `json:"supersedes,omitempty" gorm:"foreignKey:SupersedesID;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT"`
	SupersededBy         *BalanceRun              `json:"superseded_by,omitempty" gorm:"foreignKey:SupersededByID;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT"`
}

func (BalanceRun) TableName() string { return "balance_runs" }

// BeforeCreate 保证闸门原因列在全新运行时写入 JSON 空数组而不是 NULL。
func (r *BalanceRun) BeforeCreate(tx *gorm.DB) error {
	if len(r.RecalculationReasons) == 0 {
		r.RecalculationReasons = datatypes.JSON([]byte("[]"))
	}
	return nil
}

// RecalculationReason 描述一次边界证据完整性闸门命中的结构化原因。
...
// ParseRecalculationReasons 读取闸门原因链；缺失或为空时返回空切片。
func (r BalanceRun) ParseRecalculationReasons() []RecalculationReason {
	reasons := make([]RecalculationReason, 0)
	if len(r.RecalculationReasons) == 0 {
		return reasons
	}
	if err := json.Unmarshal(r.RecalculationReasons, &reasons); err != nil {
